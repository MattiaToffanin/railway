    ...
    for (int i = 0; i < 1440; ++i) {
        ...
        for (int j = 0; j < lista_Treni.size(); ++j) {
            ...
            if (i == lista_Treni[j]->getOrario(0)) {
                ...
                else if (lista_Treni[j]->getType() == moltoveloce->getType())
            ...
            if (lista_Treni[j]->getStatus() != 999 && lista_Treni[j]->getDistance() >= lista_Stazioni[lista_Treni[j]->getNextStation()]->get_distance() - 5 &&
                lista_Treni[j]->getDistance() <= lista_Stazioni[lista_Treni[j]->getNextStation()]->get_distance() && !lista_Treni[j]->isInStation()) {
            ...
            if (lista_Treni[j]->isInStation()) {
                ...
                if (lista_Treni[j]->getDistance() == lista_Stazioni[lista_Treni[j]->getNextStation()]->get_distance() &&
                    ...
                    if (i <= lista_Treni[j]->getOrario(lista_Treni[j]->getNextStation())) {
                        cout << "alle ore " << i << " il treno " << lista_Treni[j]->getId()
                             << " e' ripartito dalla stazione "
                             << lista_Stazioni[lista_Treni[j]->getNextStation()]->get_name() << " \n";
                    }else{
                        int ritardo = i - lista_Treni[j]->getOrario(lista_Treni[j]->getNextStation());
                        cout << "alle ore " << i << " il treno " << lista_Treni[j]->getId()<< " e' ripartito dalla stazione "
                             << lista_Stazioni[lista_Treni[j]->getNextStation()]->get_name() <<" con "
                             <<ritardo<<" minuti di ritardo"<< " \n";

                        lista_Treni[j]->addDelay(ritardo);
            ...
            cout<<i<<")"<<"-"<<lista_Treni[j]->getSpeed()<<"-"<<lista_Treni[j]->getDistance()<<"-"<<lista_Treni[j]->getNextStation()<<"\n"; //solo per testing
